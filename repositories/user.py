...
class UserCRUD:
    ...
    async def get_by_email(self, email) -> Optional[User]:
        user = await self.db.fetch_one(self.db_user.select().where(self.db_user.c.email == email, self.db_user.c.deleted_at == None))
    ...
    async def create(self, user_in: UserCreate) -> UserRsposneId:
        ...
        return UserRsposneId(id=await self.db.execute(user))
    ...
    async def get_users(self, page: int = 1, limit: int = 10) -> Users:
        ...
        count = len(await self.db.fetch_all(total_users))
    ...
    async def create_auth0_user(self, email: str) -> UserRsposneId:
        ...
        new_user_id = {'id': await self.db.execute(user)}
